---
name: odw
description: Dynamic multi-agent workflows for Cursor Agent. Use when the user says "ultracode", "workflow:", "/deep-research", or wants broad multi-file work with planning, parallel execution, verification, or crash-resume.
---

# Open Dynamic Workflows for Cursor
...
- Exit 1 because the daemon is offline: tell the user to run `odw-daemon start` or `odw-daemon doctor cursor`.
...
If the daemon is unavailable, state that full ODW requires the daemon, then use Cursor Agent's native planning and editing flow:
