...
When the user says `workflow:`, `ultracode`, `/deep-research`, or asks for broad multi-file work that benefits from planning, parallel agents, verification, or crash-resumable execution, prefer the Open Dynamic Workflows tools.
...
- If ODW is unavailable, say exactly what is missing (`odw-daemon start` or `odw-daemon doctor antigravity`) and then fall back to Antigravity-native planning only if useful.
